...
const highlights = [
  ...
  { value: "+12", key: "experience" as const },
  { value: "10", key: "coverage" as const },
];

const featuredServices = services.slice(0, 4).map((s) => s.titleShort);

const trustPoints = [
  t.hero.signaturePoints.selectedMaterials,
  t.hero.signaturePoints.preciseInstall,
  t.hero.signaturePoints.customDesign,
];
...
export function HomeHero() {
  ...
  const activeService =
    featuredServices[activeServiceIndex] ?? featuredServices[0];
    ...
    >
      ...
      <div className="relative mx-auto grid min-h-[92svh] w-full max-w-7xl gap-10 px-4 py-16 sm:px-6 sm:py-20 lg:min-h-0 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:gap-12 lg:px-8 lg:py-24">
        <div className="relative z-10 text-white lg:text-foreground">
          ...
          <h1
            id="hero-title"
            className="mt-7 max-w-3xl text-[36px] font-bold leading-[1.15] tracking-tight text-white drop-shadow-[0_2px_18px_rgba(0,0,0,0.55)] sm:text-5xl lg:text-[60px] lg:text-foreground lg:drop-shadow-none"
          >
            {t.hero.titleStart}{" "}
            <span className="text-[color:var(--gold)]">{t.hero.titleGold1}</span>
            <span className="block">
              {t.hero.titleMid}{" "}
              <span className="text-[color:var(--gold)]">{t.hero.titleGold2}</span>
            </span>
          </h1>
        ...
        <div className="relative hidden lg:block">
          <div className="relative aspect-[4/5] overflow-hidden rounded-[36px] border border-border/70 shadow-[0_40px_120px_-40px_rgba(32,27,21,0.4)]">
            <Image
              src={marbleImages.services.supply}
              alt="تشطيب رخام فاخر داخلي من أعمال فيو"
              fill
              priority
              fetchPriority="high"
            ...
            <div
              aria-hidden
              className="absolute inset-0 bg-gradient-to-t from-background/75 via-background/10 to-transparent"
            />
          ...
          <div className="absolute -bottom-8 -left-8 w-[240px] rounded-[30px] border border-border/70 bg-card/90 p-4 shadow-[0_35px_90px_-45px_rgba(32,27,21,0.55)] backdrop-blur">
            ...
            <div className="relative mt-4 aspect-[4/3] overflow-hidden rounded-2xl">
              <Image
                ...
                alt="مغسلة رخام تفصيل من أعمال فيو"
                fill
                sizes="240px"
                className="object-cover"
              />
